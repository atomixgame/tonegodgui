...
import tonegod.gui.framework.core.ActionManager;
...
public class Screen implements ElementManager, Control, RawInputListener {
     ...
     *
     * @param app A JME Application
     * @param styleMap A path to the style_map.xml file containing the custom
     * theme information
     */
     ...
     *
     * @param stepMajor Return the z-order incremented by a major step if true,
     * a minor step if false
     * @return float zOrder
     */
    ...
    private void androidTouchUpEvent(TouchEvent evt) {
        ...
        } else //	handleAndroidMenuState(target);
        if (eventAnimElement != null) {
            if (eventAnimElement instanceof MouseButtonListener) {
                MouseButtonEvent mbEvt = new MouseButtonEvent(0, true, (int) touchXY.x, (int) touchXY.y);
...
//	@Override
    ...
    /**
     * Returns a pointer to the ActionManager. The ActionManager is a time based
     * queuing for TemporalActions used with
     *
     * @Transformable ( See
     * @AnimElement
     * @QuadData )
    ...
    public void setSpatial(Spatial spatial) {
        ...
        if (spatial != null) {
            ...
            t0neg0dGUI.addControl(actionManager);
            initModalBackground();
...
//			if (isAndroid()) initVirtualKeys();
     ...
     * @param volume float the volume to play the instance at (effected by
     * global volume)
     */
