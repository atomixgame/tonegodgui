...
public class ScaleByAction extends TemporalAction {
    ...
    private void reverseQuad() {
        if (initX == -1) {
            initX = quad.getScale().x;
        }
        if (initY == -1) {
            initY = quad.getScale().y;
        }
    }

    private void reverseTransform() {
        if (initX == -1) {
            initX = ((Node) quad).getLocalScale().getX();
        }
        if (initY == -1) {
            initY = ((Node) quad).getLocalScale().getY();
        }
    }
    ...
    private void updateQuad() {
        quad.setScale(quad.getScale().x + (amountX * nextPercent), quad.getScale().y + (amountY * nextPercent));
    }
